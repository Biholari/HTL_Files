...
SELECT *
FROM Vermietungen
WHERE DATEDIFF(DAY, Startdatum, Enddatum) < 7;
...
SELECT VermietungID, Preis / DATEDIFF(DAY, Startdatum, Enddatum) AS 'Tägliche Kosten'
FROM Vermietungen;
...
SELECT *
FROM Inspektionen
WHERE Beschreibung LIKE '%erneuern%' OR Beschreibung LIKE '%wechsel%'
